...
/**
 * Basic example demonstrating Frida Java bindings usage. This example shows version information,
 * device enumeration, and process listing.
 */
    ...
    try {
      ...
      try (DeviceManager deviceManager = new DeviceManager()) {
        ...
        for (Device device : devices) {
          System.out.printf(
              "  - %s (Type: %s, ID: %s)%n", device.getName(), device.getType(), device.getId());
        }
