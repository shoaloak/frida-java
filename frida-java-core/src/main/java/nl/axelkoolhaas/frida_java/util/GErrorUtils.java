...
public class GErrorUtils {
  ...
  static {
    G_ERROR_FREE =
        FridaLibraryLoader.findFunction(
            "g_error_free", FunctionDescriptor.ofVoid(ValueLayout.ADDRESS));
  }
  ...
  /**
   * Get the error message from a GError GError struct: { GQuark domain (4 bytes); gint code (4
   * bytes); gchar *message (pointer); }
   *
