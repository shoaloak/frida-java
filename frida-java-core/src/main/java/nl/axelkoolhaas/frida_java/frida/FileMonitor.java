...
public class FileMonitor implements AutoCloseable {
  ...
  static {
    ...
    FRIDA_FILE_MONITOR_DISABLE_SYNC =
        FridaLibraryLoader.findFunction(
            "frida_file_monitor_disable_sync",
            FunctionDescriptor.ofVoid(
                ValueLayout.ADDRESS, ValueLayout.ADDRESS, ValueLayout.ADDRESS));
   ...
   *
   * <p>Available signals: - "change": Emitted when a file change is detected Callback signature:
   * void onFileChange(String changedFile, String otherFile, String changeType)
   *
  ...
  public void on(String signalName, Object callback) {
    ...
    if (!"change".equals(signalName)) {
      throw new IllegalArgumentException(
          "Unknown signal: " + signalName + ". Only 'change' is supported.");
    }
