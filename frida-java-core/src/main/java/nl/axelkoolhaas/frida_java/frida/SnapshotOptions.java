...
public class SnapshotOptions implements AutoCloseable {
  ...
  static {
    ...
    FRIDA_SNAPSHOT_OPTIONS_NEW =
        FridaLibraryLoader.findFunction(
            "frida_snapshot_options_new", FunctionDescriptor.of(ValueLayout.ADDRESS));
  ...
  public String getWarmupScript() {
    try {
      MemorySegment warmupPtr =
          (MemorySegment) FRIDA_SNAPSHOT_OPTIONS_GET_WARMUP_SCRIPT.invoke(optionsPtr);
