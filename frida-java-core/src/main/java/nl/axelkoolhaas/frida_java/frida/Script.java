...
public class Script implements AutoCloseable {
  ...
  /**
   * Register callbacks for script events. Available signals: - "message" with callback as
   * MessageCallback - "destroyed" with callback as VoidCallback or Runnable
   *
    ...
    if ("message".equals(signalName)) {
      if (!(callback instanceof SignalCallbacks.MessageCallback)) {
        throw new IllegalArgumentException(
            "Callback must be a MessageCallback for 'message' signal");
      }
      ...
      if (!(callback instanceof SignalCallbacks.VoidCallback) && !(callback instanceof Runnable)) {
        throw new IllegalArgumentException(
            "Callback must be a VoidCallback or Runnable for 'destroyed' signal");
      }
  ...
  /**
   * Internal method to create and execute RPC calls <hr> Note: Unlike the Go implementation which
   * uses sync.Pool for channel pooling, we don't pool CompletableFuture objects here. Modern Java's
   * JVM (especially Java 11+) handles short-lived object allocation extremely efficiently through:
   * - Escape analysis (stack allocation for thread-local objects) - TLAB (Thread-Local Allocation
   * Buffers) for near-zero cost allocation - Efficient GC (G1/ZGC) that handles high allocation
   * rates with minimal pause times <br>
   * Pooling would only be beneficial if profiling shows >10,000 RPC calls/second causing GC
   * pressure, which is extremely rare in typical Frida workflows.
   */
