...
public class Compiler implements AutoCloseable {
  ...
  static {
    ...
    FRIDA_COMPILER_NEW =
        FridaLibraryLoader.findFunction(
            "frida_compiler_new", FunctionDescriptor.of(ValueLayout.ADDRESS, ValueLayout.ADDRESS));
  ...
  /** Create a new compiler instance. Creates and owns a DeviceManager internally. */
  public Compiler() {
  ...
  /**
   * Create a new compiler instance with the given device manager. The caller retains ownership of
   * the DeviceManager.
   *
  ...
  public String build(String entrypoint, CompilerOptions options) {
    ...
    try (Arena arena = Arena.ofConfined()) {
      ...
      MemorySegment resultPtr =
          (MemorySegment)
              FRIDA_COMPILER_BUILD_SYNC.invoke(
