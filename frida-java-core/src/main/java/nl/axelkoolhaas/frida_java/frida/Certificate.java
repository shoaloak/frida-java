...
public final class Certificate {
    ...
    private static final DateTimeFormatter JAVA_TIME_FORMAT = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss");

    static {
        Frida.ensureInitialized();
        G_TLS_CERTIFICATE_GET_ISSUER_NAME = FridaLibraryLoader.findFunction(
                "g_tls_certificate_get_issuer_name",
                FunctionDescriptor.of(ValueLayout.ADDRESS, ValueLayout.ADDRESS));
        G_TLS_CERTIFICATE_GET_SUBJECT_NAME = FridaLibraryLoader.findFunction(
                "g_tls_certificate_get_subject_name",
                FunctionDescriptor.of(ValueLayout.ADDRESS, ValueLayout.ADDRESS));
        G_TLS_CERTIFICATE_GET_NOT_VALID_BEFORE = FridaLibraryLoader.findFunction(
                "g_tls_certificate_get_not_valid_before",
                FunctionDescriptor.of(ValueLayout.ADDRESS, ValueLayout.ADDRESS));
        G_TLS_CERTIFICATE_GET_NOT_VALID_AFTER = FridaLibraryLoader.findFunction(
                "g_tls_certificate_get_not_valid_after",
                FunctionDescriptor.of(ValueLayout.ADDRESS, ValueLayout.ADDRESS));
        G_DATE_TIME_FORMAT = FridaLibraryLoader.findFunction(
                "g_date_time_format",
    ...
    public String getIssuerName() {
        try {
            MemorySegment namePtr = (MemorySegment) G_TLS_CERTIFICATE_GET_ISSUER_NAME.invokeExact(certPtr);
    ...
    public String getSubjectName() {
        try {
            MemorySegment namePtr = (MemorySegment) G_TLS_CERTIFICATE_GET_SUBJECT_NAME.invokeExact(certPtr);
            return FridaNativeUtils.memorySegmentToStringAndFree(namePtr);
        } catch (Throwable t) {
    ...
    public LocalDateTime getNotValidBefore() {
        try {
            MemorySegment dateTimePtr = (MemorySegment) G_TLS_CERTIFICATE_GET_NOT_VALID_BEFORE.invokeExact(certPtr);
            try (Arena arena = Arena.ofConfined()) {
                MemorySegment formatStr = arena.allocateFrom(C_TIME_FORMAT);
                MemorySegment formattedPtr = (MemorySegment) G_DATE_TIME_FORMAT.invokeExact(dateTimePtr, formatStr);
    ...
    public LocalDateTime getNotValidAfter() {
        try {
            MemorySegment dateTimePtr = (MemorySegment) G_TLS_CERTIFICATE_GET_NOT_VALID_AFTER.invokeExact(certPtr);
            try (Arena arena = Arena.ofConfined()) {
                MemorySegment formatStr = arena.allocateFrom(C_TIME_FORMAT);
                MemorySegment formattedPtr = (MemorySegment) G_DATE_TIME_FORMAT.invokeExact(dateTimePtr, formatStr);
                String formatted = FridaNativeUtils.memorySegmentToStringAndFree(formattedPtr);
