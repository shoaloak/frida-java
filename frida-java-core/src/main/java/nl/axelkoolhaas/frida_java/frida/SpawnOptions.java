...
public class SpawnOptions implements AutoCloseable {
  ...
  static {
    ...
    FRIDA_SPAWN_OPTIONS_NEW =
        FridaLibraryLoader.findFunction(
            "frida_spawn_options_new", FunctionDescriptor.of(ValueLayout.ADDRESS));
  ...
  public List<String> getArgv() {
    try (Arena arena = Arena.ofConfined()) {
      ...
      MemorySegment argvPtr =
          (MemorySegment) FRIDA_SPAWN_OPTIONS_GET_ARGV.invoke(optionsPtr, countPtr);
  ...
  public List<String> getEnvp() {
    try (Arena arena = Arena.ofConfined()) {
      MemorySegment countPtr = arena.allocate(ValueLayout.JAVA_INT);
      MemorySegment envpPtr =
          (MemorySegment) FRIDA_SPAWN_OPTIONS_GET_ENVP.invoke(optionsPtr, countPtr);
  ...
  public List<String> getEnv() {
    try (Arena arena = Arena.ofConfined()) {
      MemorySegment countPtr = arena.allocate(ValueLayout.JAVA_INT);
      MemorySegment envPtr =
          (MemorySegment) FRIDA_SPAWN_OPTIONS_GET_ENV.invoke(optionsPtr, countPtr);
