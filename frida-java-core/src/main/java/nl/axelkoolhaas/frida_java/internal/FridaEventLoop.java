...
public class FridaEventLoop {
    ...
    static {
        ...
        G_MAIN_LOOP_UNREF = FridaLibraryLoader.findFunction("g_main_loop_unref",
                FunctionDescriptor.ofVoid(ValueLayout.ADDRESS));
        G_IDLE_ADD = FridaLibraryLoader.findFunction("g_idle_add",
    ...
    /**
     * Internal utility: Executes a task on the GLib thread.
     * If called from the GLib thread, runs immediately.
     * If called from elsewhere, schedules via g_idle_add and waits (blocking).
     */
    ...
    private static MemorySegment createIdleStub() {
        try {
            MethodHandle handler = MethodHandles.lookup()
                    .findStatic(FridaEventLoop.class, "idleCallback",
                            MethodType.methodType(boolean.class, MemorySegment.class));
            return Linker.nativeLinker().upcallStub(handler,
                    FunctionDescriptor.of(ValueLayout.JAVA_BOOLEAN, ValueLayout.ADDRESS), Arena.global());
        } catch (Exception e) {
