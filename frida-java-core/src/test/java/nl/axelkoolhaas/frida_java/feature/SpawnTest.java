...
/**
 * Test class for Frida Process spawn functionality. Tests process spawning, resuming, killing, and
 * spawn enumeration.
 */
...
public class SpawnTest {
  ...
  void testSpawnSimpleProcess() {
    try (DeviceManager deviceManager = new DeviceManager()) {
      ...
      if (spawnedPid > 0) {
        System.out.println(
            "Successfully spawned process '" + usedProgram + "' with PID: " + spawnedPid);
  ...
  void testSpawnProcessWithArgs() {
    try (DeviceManager deviceManager = new DeviceManager()) {
      ...
      try {
        ...
        abort("Could not spawn process with args: " + e.getMessage());
      }
    }
  }
  ...
  void testKillProcess() {
    try (DeviceManager deviceManager = new DeviceManager()) {
      ...
      try {
        ...
        abort("Could not test kill process: " + e.getMessage());
      } catch (InterruptedException e) {
        Thread.currentThread().interrupt();
        fail("Test was interrupted");
      }
  ...
  void testInvalidSpawn() {
    try (DeviceManager deviceManager = new DeviceManager()) {
      ...
      assertThrows(
          ...
          () -> localDevice.spawn("/nonexistent/program"),
          "Should throw exception for non-existent program");
