...
/**
 * Test class for Device-related Frida bindings. Tests device enumeration, device properties, and
...
public class DeviceTest {
  ...
  void testDeviceProperties() {
    try (DeviceManager deviceManager = new DeviceManager()) {
      ...
      System.out.printf("Device - ID: %s, Name: %s, Type: %s, Lost: %s%n", id, name, type, isLost);
    }
  }
  ...
  void testFindProcessByName() {
    try (DeviceManager deviceManager = new DeviceManager()) {
      ...
      for (Process p : processes) {
        ...
        if (name.equals("kernel_task")
            || name.equals("launchd")
            || name.equals("init")
            || name.equals("systemd")
            || name.equals("System")
            || name.equals("svchost.exe")) {
          targetProcess = p;
          break;
        }
      }
  ...
  void testSpawnGatingEnableDisable() {
    try (DeviceManager deviceManager = new DeviceManager()) {
      ...
      try {
        ...
        System.out.println(
            "Spawn gating not supported (macOS with SIP or restricted platform): "
                + e.getMessage());
  ...
  void testInjectLibraryFile() {
    ...
    if (!isUnix) {
      System.out.println(
          "Skipping library injection test - Unix-only test (TODO: add Windows support)");
    ...
    try (DeviceManager deviceManager = new DeviceManager()) {
      ...
      System.out.println(
          "Library file injection API available (actual test requires test library)");
    }
  }
  ...
  void testInjectLibraryBlob() {
    ...
    if (!isUnix) {
      System.out.println(
          "Skipping library blob injection test - Unix-only test (TODO: add Windows support)");
      Assumptions.assumeTrue(false, "Unix-only test");
      return;
    }
    ...
    try (DeviceManager deviceManager = new DeviceManager()) {
      ...
      System.out.println(
          "Library blob injection API available (actual test requires valid library bytes)");
    }
  }
  ...
  void testOpenChannel() {
    ...
    try (DeviceManager deviceManager = new DeviceManager()) {
      ...
      System.out.println(
          "Channel opening API available (actual test requires valid Frida service)");
    }
  }
  ...
  void testOpenService() {
    ...
    try (DeviceManager deviceManager = new DeviceManager()) {
      ...
      System.out.println(
          "Service opening API available (actual test requires valid Frida service)");
    }
  }
}
